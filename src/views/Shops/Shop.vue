<template>
  <div class="shop" v-if="shopInfo">
    ...
    <div class="index-rst">
      <div class="rst-name">
        <span @click="shopInfoModel = true">{{ shopInfo.rst.name }}</span>
...
<script>
import infoModle from '../../components/shops/infoModle'
...
export default {
  ...
  methods: {
    async getData() {
      const { data: res } = await this.$http('/api/profile/batch_shop')
     /*  console.log(res) */
      this.shopInfo = res
    },
  },

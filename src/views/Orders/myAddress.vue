...
<div class="address-view">
  <div class="address-card" v-for="(address,index) in allAddress" :key="index">
   ...
   <div class="address-card-edit">
     ...
     <i @click="handleDelect(address,index)"  class="fa fa-close"></i>
   </div>
  </div>
</div>

<!-- 新增收货地址 -->
<div class="address-view-bottom" @click="$router.push({
  name:'addAddress',
  params:{
    title:'添加地址',
    addressInfo:{
...
methods:{
 async getallAddress(){
      const {data:res} =await  this.$http(`/api/user/user_info/${localStorage.ele_login}`)
    /*   console.log(res); */
  ...
  handleDelect(address,index){
       this.$http.delete(`/api/user/address/${localStorage.ele_login}/${address._id}`).then(
         res=>{
  ...
  setAddressInfo(address,index){
    ...
    this.$store.dispatch('setUser',address)

<template>
  <div class="Order">
    <div class="order-card-body" v-for="(order,index) in orderList" :key="index">
     <div class="order-card-wrap" @click="$router.push({name:'orderInfo',params:{orderInfo:order}})" v-if="order.orderInfo">
       ...
       <div class="order-card-content">
         ...
         <div class="order-card-detail">
           <p class="detail">{{order.orderInfo.selectfoods[1].name}}</p>
           <p class="price">￥{{order.totalPrice}} </p>
         </div>
       </div>
     </div>
     <div class="order-card-bottom">
       <button class="cardbutton" @click="$router.push('/shop')">
       再来一单
       </button>
     </div>
    </div>
...
methods:{
 async getDate(){
       const {data:res} = await this.$http(`/api/user/orders/${localStorage.ele_login}`)
